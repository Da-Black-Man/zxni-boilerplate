...
// import "../vendors/charming.min.js";
// import "../vendors/anime.min.js";
...
const settings = {
  ...
  description: { duration: 500, delay: 400, easing: "easeOutExpo" },
...
class Entry {
  ...
  init() {
    ...
    // more box
    this.DOM.more = this.DOM.el.querySelector(
      ".section__more > .section__more-inner"
    );
    // expander
    ...
    this.DOM.facts = {
      ...
      items: Array.from(
        this.DOM.el.querySelectorAll(".section__facts > .section__facts-item")
      ),
    };
  }
  show(direction) {
      ...
      opacity: {
        ...
        delay: (target, index) =>
          index
            ? this.isHidden
              ? 100
              : settings.more.duration * 0.5 + settings.more.delay
            : this.isHidden
            ? settings.more.duration + settings.more.delay
            : settings.more.delay,
      },
    }).finished;
  }
    ...
    return anime({
      ...
      delay: (target, index) => {
        return !this.isHidden
          ? index * 40 + settings.facts.duration * 0.5 + settings.facts.delay
          : index * 40 + settings.facts.delay;
      },
...
class Slideshow {
  ...
  init() {
    // DOM elements.
    this.DOM.menuCtrl = this.DOM.el.querySelector(
      ".sections__header > button.button-menu"
    );
    this.DOM.menu = {
      wrapper: this.DOM.el.querySelector(".menu"),
      items: Array.from(
        this.DOM.el.querySelectorAll(".menu > .menu__inner > .menu__item")
      ),
      menuCtrls: {
        toggle: this.DOM.el.querySelector(".menu > .menu__toggle"),
        open: this.DOM.el.querySelector(
          ".menu > .menu__toggle > .menu__toggle-inner--open"
        ),
        close: this.DOM.el.querySelector(
          ".menu > .menu__toggle > .menu__toggle-inner--close"
        ),
      },
    };
    ...
    this.DOM.factsCtrls = {
      ...
      more: this.DOM.factsContainer.querySelector(
        ".facts__toggle > .facts__toggle-inner--more"
      ),
      less: this.DOM.factsContainer.querySelector(
        ".facts__toggle > .facts__toggle-inner--less"
      ),
    };
    this.DOM.previewCloseCtrl = this.DOM.factsContainer.querySelector(
      ".button-contentclose"
    );
  ...
  layout() {
    ...
    const factHeight =
      factEl.getBoundingClientRect().height +
      parseFloat(window.getComputedStyle(factEl).marginBottom);
    const paddingFactsStyle = window.getComputedStyle(
      this.currentEntry.DOM.facts.wrapper
    );
    const paddingFacts =
      parseFloat(paddingFactsStyle.paddingTop) +
      parseFloat(paddingFactsStyle.paddingBottom);
  ...
  initEvents() {
    ...
    // Facts Container
    this.DOM.factsCtrls.toggle.addEventListener("click", () =>
      this.toggleFactsContainer()
    );
    ...
    for (let i = 0; i <= this.entriesTotal - 1; ++i) {
      ...
      entry.DOM.facts.items
        .filter((fact) =>
          fact.classList.contains("section__facts-item--clickable")
        )
        .forEach((clickableFact) =>
          clickableFact.addEventListener("click", () =>
            this.preview(clickableFact.dataset.gallery)
          )
        );
    }

    // Close preview
    this.DOM.previewCloseCtrl.addEventListener("click", () =>
      this.closePreview()
    );
    ...
    this.onResize = () => {
      ...
      if (this.gallery) {
        ...
        this.toggleGallery(this.gallery, { duration: 1, opacity: 0 }).then(
          () => (this.gallery = null)
        );
      }
    ...
    const newPos = (this.currentPos =
      this.direction === "next"
        ? this.currentPos < this.entriesTotal - 1
          ...
          : 0
        : (this.currentPos =
            this.currentPos > 0 ? this.currentPos - 1 : this.entriesTotal - 1));
  ...
  toggleFactsContainer() {
    if (
      this.isFactsAnimating ||
      (this.isEntriesAnimating && !this.isFactsOpen)
    ) {
      return;
    }
      ...
      Object.assign(
        {
          ...
          translateY: !this.isFactsOpen
            ? [this.factsTranslation, 0]
            : this.factsTranslation,
          scaleX: [0.54, 0.54],
        },
        animeconfig
      )
      ...
      Object.assign(
        {
          ...
          translateY: !this.isFactsOpen
            ? [this.factsTranslation, 0]
            : this.factsTranslation,
          opacity: (target, index) =>
            !this.isFactsOpen ? 1 : index > 1 ? 0 : 1,
        },
        animeconfig
      )
    );
      ...
      Object.assign(
        {
          ...
          opacity: (target, index) =>
            index ? (!this.isFactsOpen ? 1 : 0) : !this.isFactsOpen ? 0 : 1,
        },
        animeconfig
      )
    );
